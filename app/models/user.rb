...
class User
  ...
  #validates_format_of :first_name, :last_name, :user_name, with:/\A[a-z0-9_\-@\.]*\z/i
  validates_length_of :first_name, :last_name, :user_name, maximum: NAME_MAX_LENGTH
  validates_format_of :email, with: /\A([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})\z/i
  #validates_length_of :password, :password_confirmation, minimum: PASSWORD_MIN_LENGTH, allow_blank: true
  ...
  class << self
    ...
    def find_by_username(user_name)
      ...
      if user.nil?
        user = User.find_by(user_name: /^#{user_name}$/i)
      end
      user
    end
  end
