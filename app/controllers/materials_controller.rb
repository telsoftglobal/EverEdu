  ...
  def quickupdate
    begin
      ...
      if material.nil?
        ...
        if !material.update_attributes(:material_name => params[:material][:material_name],:material_url => params[:material][:material_url], :description => params[:material][:description],:material_type => params[:material][:material_type_id])
          respond_to do |format|
            if material.errors
              flash.now[:error] = material.errors.full_messages.to_sentence(:last_word_connector => ', ')
            else
