class RoadmapController < ApplicationController
  ...
  def get_more_roadmap
    begin
      ...
      if !@user_categories = UserCategory.get_more_roadmaps(session[:user_id], last_roadmap_id)
        if @user_category.errors
          flash.now[:error] = @user_category.errors.full_messages.to_sentence(:last_word_connector => ', ')
        else
          flash.now[:error] = t('roadmap.msg_create_roadmap_fail')
  ...
  def update
    begin
      ...
      if @user_category.nil?
        ...
        @user_category.update_attributes(:level_id => level_id)
        respond_to do |format|
          format.js
          format.html
        end
  ...
  def delete
    begin
      ...
      if @user_category.nil?
        ...
        UserCategory.delete_all(_id: BSON::ObjectId.from_string(roadmap_id))
        respond_to do |format|
          format.js
          format.html
        end
