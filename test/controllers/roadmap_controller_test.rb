...
class RoadmapControllerTest < ActionController::TestCase
  ...
  test "check" do
    user_category = UserCategory.where(user_id: nil).first
  ...
  test "view my roadmap" do
    ...
    user_categories.each do |category_user|
      ...
      assert_select 'div#roadmap-chart' + category_user.id
    end
    puts '-----------Output on database:------------'
    user_categories = user_categories = UserCategory.where(user_id: user.id).order_by(created_at: -1).limit(5)
    user_categories.each do |category_user|
      puts category_user.category.category_name
      puts category_user.level.level_name
    end
  ...
  test "view more roadmap" do
    ...
    puts '-----------Output on database:------------'
    user_categories = user_categories = UserCategory.where(user_id: user.id).lt(_id: last_roadmap_id).order_by(created_at: -1).limit(5)
    user_categories.each do |category_user|
      puts category_user.category.category_name
      puts category_user.level.level_name
    end
